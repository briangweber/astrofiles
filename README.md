...
If your imaging train remains assembled between sessions, you may be able to reuse flats for an extended period of time. You will need to shoot new flats if you adjust the imaging train, clean the optics, or dust/debris accumulates.

To configure this functionality, create a file called `flats.json` in the shared calibration root. The structure looks like this:

```
{
  "ATR585M": [
    { "filter": "R", "startDate": "2025-08-26", "endDate": "2025-10-02", "flatDate": "2025-08-26" },
    { "filter": "R", "startDate": "2025-10-03", "flatDate": "2025-10-03" }
  ],
  "168C": [
    { "filter": "NBZ", "startDate": "2025-08-26", "endDate": "2025-10-02", "flatDate": "2025-08-26" },
    { "filter": "NBZ", "startDate": "2025-10-03", "flatDate": "2025-10-03" }
  ]
}
```

The top-level field is the camera name. Each entry must have the filter, startDate, flatDate (this is the folder to use for flats), and an optional endDate. If no endDate is provided, the flats are considered to be still usable. During the calibration linking process, the script will check for an applicable date range for each filter and link the appropriate flat files with a preference toward masterFlat files. If no suitable date ranges are found, the script will fall back to looking for a flats directory for the same date of the session.
